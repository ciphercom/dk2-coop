...
The diagnostic bridge, development-only availability overrides, investigation captures, protocol/OOS instrumentation additions and source-workspace test profiles are excluded from the co-op port. Existing upstream facilities are not a prerequisite for playing co-op. Regression tests that express gameplay or package contracts remain part of the normal build.
